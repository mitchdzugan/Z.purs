module Z.Z.Lens.Barlow.Parser where

import Z.Z.Lens.Barlow.Types
import Prim.Symbol as Symbol

class ParsePercentage :: forall k. Symbol -> Symbol -> k -> Symbol -> Constraint
class
  ParsePercentage (head :: Symbol) (tail :: Symbol) out (rest :: Symbol)
  | head tail -> out rest

class
  ParsePercentageSymbol
    (head :: Symbol)
    (tail :: Symbol)
    (out :: Symbol)
    (rest :: Symbol)
...
class
  ParseSymbol (string :: Symbol) (attributes :: TList)
  | string -> attributes

class
  Parse1Symbol (head :: Symbol) (tail :: Symbol) (out :: TList)
  | head tail -> out
...
else instance parserecordfieldQuestionMark ::
  ( Symbol.Cons "?" t out
  ) =>
  ParseRecordField "?" t "" out
else instance parserecordfieldLeftArrow ::
  ( Symbol.Cons "<" t out
  ) =>
  ParseRecordField "<" t "" out
else instance parserecordfieldRightArrow ::
  ( Symbol.Cons ">" t out
  ) =>
  ParseRecordField ">" t "" out
else instance parserecordfieldPlus ::
  ( Symbol.Cons "+" t out
  ) =>
  ParseRecordField "+" t "" out
else instance parserecordfieldExclamationMark ::
  ( Symbol.Cons "!" t out
  ) =>
  ParseRecordField "!" t "" out
